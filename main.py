...
goal2 = pygame.Rect(screen.get_width() - 5, screen.get_height() / 2 - goalheight, 5, 100)
...
bluepadimg = pygame.image.load('./redpad.png')
...
def gameLoop():
    ...
    while not gameExit:
        ...
        for event in pygame.event.get():
            up2, down2, left2, right2, up, down, left, right = 0, 0, 0, 0, 0, 0, 0, 0
            #print(event)
            if event.type == pygame.QUIT:
                gameExit = True
            if event.type == pygame.KEYDOWN:
        ...
        if (disc.x <= goalwidth) and (disc.y <= screen.get_height() / 2 + goalheight) and (
                disc.y >= screen.get_height() / 2 - goalheight):
            score2 += 1
            serveDirection = -1
            resetPuck()
        elif (disc.x >= screen.get_width() - goalwidth - 15) and (
        ...
        # Render Logic
        screen.fill(black)
        message_to_screen("Player 1", white, -200, -150, "small")
        message_to_screen(str(score1), white, -150, -30, "small")
        message_to_screen("Player 2", white, -200, 150, "small")
        message_to_screen(str(score2), white, -150, 30, "small")
        pygame.draw.rect(screen, (255, 100, 100), paddle1)
        pygame.draw.rect(screen, (20, 20, 100), paddle2)
        pygame.draw.rect(screen, light_violet, goal1)
        pygame.draw.rect(screen, light_violet, goal2)
        ...
        pygame.draw.line(screen, red, (0, 0), (screen.get_width() / 2 - 5, 0), 5)
        pygame.draw.line(screen, red, (0, screen.get_height()), (screen.get_width() / 2 - 5, screen.get_height()), 5)
        ...
        pygame.draw.line(screen, red, (0, 0), (0, screen.get_height() / 2 - goalheight), 5)
        pygame.draw.line(screen, red, (0, screen.get_height() / 2 + goalheight), (0, screen.get_height()), 5)
